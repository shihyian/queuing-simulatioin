...
function in_animation(target) {
	...
	duration = (moveX / (speed * speed_times.val())) * 1000;
	if (duration < 200) {
		duration = duration * 1.5; // 讓短動畫稍微延長
	}

	// 進場動畫
	if (speed_times.val() <= 20){
		testAni = anime({
			...
			complete: function () {
				let customerElem = $(`#cus-${target.order}`);
				if (customerElem.length > 0) {
					customerElem.appendTo("#queuing");

					let correctX = 550 - 35 * ($("#queuing").children().length - 1);
					anime({
						targets: `#cus-${target.order}`,
						translateX: correctX,
						easing: "linear",
					...
					console.warn(`Customer #cus-${target.order} disappeared!`);
				}
			},
		});
		testAni.play();
...
function out_animate() {
	// 正在離開的人移到leaving群組處理
	let leavingCustomer = $(`#cus-${i}`);
    if (leavingCustomer.length === 0) {
        console.warn(`Customer #cus-${i} already removed!`);
        return;
    }

    leavingCustomer.appendTo("#leaving");
	...
	if (speed_times.val() <= 20){
		outAni = anime({
			targets: `#cus-${customerId}`,
			translateX: "+=50",
			easing: "linear",
			duration: Math.max(200 / speed_times.val(), 100),
			complete: function () {
				// 只有當 `#cus-${i-1}` 仍存在時才刪除
				let targetElem = $(`#cus-${customerId}`);
	...
	if (i >= cus_num.val() && queuing_count == 0) {
		$("#queuing-length").remove();
		$("#text-area").append('<span id="congratulation">All customers leaves!<br>Please reset and start again!</span>');
...
function ani_reset() {
	...
	$("#text-area").empty();
	$("#text-area").append('<span id="queuing-length">Queuing Length: <span id="cus-num">0</span></span>');
...
function generate(timestamp) {
	...
	if (i >= cus_num.val() && queuing_count == 0){
		console.log('customers all leave');
		return;
	}
